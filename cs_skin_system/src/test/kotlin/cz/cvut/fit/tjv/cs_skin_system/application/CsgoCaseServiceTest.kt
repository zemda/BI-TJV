...
class CsgoCaseServiceTest {
    ...
    fun `updateCsgoCase add already existent skin to case test`() {
        ...
        assertEquals(
                "Skin with id ${skin.id} is already in the case with id ${existingCase.id}",
                exception.message
        )
    ...
    fun `updateCsgoCase remove skin from case which is not there test`() {
        ...
        assertEquals(
                "No skin with id ${skin.id} found in the case with id ${existingCase.id}",
                exception.message
        )
    ...
    fun `updateCsgoCase add multiple skins test`() {
        ...
        val updatedCase =
                csgoCaseService.updateCsgoCase(
                        existingCase.id,
                        listOf(skin1.id, skin2.id, skin3.id),
                        true
                )
        ...
        val csgoCase =
                caseRepo.findById(updatedCase.id).orElseThrow {
    ...
    fun `updateCsgoCase update case price test`() {
        ...
        `when`(caseRepo.findById(1L)).thenReturn(Optional.of(existingCase))
        `when`(caseRepo.save(any(CsgoCase::class.java)))
                .thenReturn(
                        CsgoCase().apply {
                            id = 1L
                            price = newPrice
